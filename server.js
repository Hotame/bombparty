...
function update(label, type, data) {
  switch (type) {
    case MessageType.Rename:
      ...
      if (Object.values(state.players).includes(new_name)) {
        return;
      }
      state.players[label] = new_name;
      break;
    case MessageType.EnterGame:
      if (state.queue[label]) {
        return;
      }
    ...
    case MessageType.Typed:
      if (!state.started || state.game.order[state.game.turn] != label) {
        return;
      }
    ...
    case MessageType.Submit:
      ...
      if (
        dictionarySet.has(data) &&
        !played[data] &&
        data.includes(state.game.query)
      ) {
        for (const letter of data) {
          if (letter in state.game.letters[label]) {
            state.game.letters[label][letter] = Math.max(
              0,
              state.game.letters[label][letter] - 1
            );
          }
        }
    ...
    case MessageType.StartGameRequest:
      ...
      state.game = {
        ...
        lives: Object.fromEntries(
          order.map((id) => [id, state.settings.lives])
        ),
...
async function update_settings(elem) {
  ...
  switch (elem.id) {
    ...
    case "custom_dictionary_input":
      ...
      break;
    default:
      console.log("Error updating:", elem);
  }
}
...
function deadlineTimeout(delay) {
  if (lastDeadlineID) {
    clearTimeout(lastDeadlineID);
  }
  lastDeadlineID = setTimeout(() => {
    if (
      state.started &&
      state.game?.deadline &&
      state.game.deadline < Date.now()
    ) {
      let deadline_label = state.game.order[state.game.turn];
      state.game.lastSolve[deadline_label] =
        String.fromCodePoint(55357) + String.fromCodePoint(56468) + `(${word})`;
...
function removePlayerFromGame(label) {
  delete state.queue[label];
  if (!(state.started && state.game.order.includes(label))) {
    return;
  }
...
fetch(`${BasePath}dictionaries/${language}.txt`)
