...
function handleData(data) {
  if (data.type == MessageType.Ping) {
    last_server_message = data.data;
    return conn.send(Pong(Date.now()));
  }
  if (data.type != MessageType.StateUpdate) {
    return debug(data);
  }
  if (!state) {
    insert_game_html();
...
peer.on("open", (id) => {
  ...
  peer.on("close", () => {
    ...
    document.getElementsByTagName("body")[0].innerHTML =
      "<h1>Disconnected from server</h1><h2><a href='./.'>Host one yourself?<a></h2>";
  });
...
function checkConnection() {
  if (!label) {
    document.getElementsByTagName("body")[0].innerHTML = `
  <div style="margin-right: 200px; text-align: center; align-items: center;">
    <h1>Could not connect to server</h1>
    <br><h2><a href="./.">Host one yourself?</a></h2>
  </div>
`;
